...
public class MysqlProjectDao implements ProjectDao {
	...
	public List<Project> selectList() throws Exception {
		...
		try {
			...
			rs = stmt.executeQuery("SELECT PNO,PNAME,CONTENT,STA_DATE,END_DATE,STATE FROM PROJECTS ORDER BY PNO ASC");
			while(rs.next()) {
	...
	public Project selectOne(int no) throws Exception {
		...
		try {
			conn = ds.getConnection();
			stmt = conn.prepareStatement("SELECT PNO,PNAME,CONTENT,STA_DATE,END_DATE,STATE,TAGS FROM PROJECTS WHERE PNO=?");
			stmt.setInt(1, no);
			rs = stmt.executeQuery();
			if(rs.next()) {
				System.out.println(rs.getString("PNO"));
				return new Project ().setContent(rs.getString("CONTENT")).setTitle(rs.getString("PNAME")).setStartDate(rs.getDate("STA_DATE"))
						.setEndDate(rs.getDate("END_DATE")).setTags(rs.getString("TAGS")).setState(rs.getInt("STATE")).setNo(rs.getInt("PNO"));
			}
		}catch(Exception e) {
			try {if(rs!=null) rs.close();}catch(Exception rsE) {}
			try {if(stmt!=null) stmt.close();}catch(Exception stmtE) {}
	...
	public int update(Project project) throws Exception {
		...
		try {
			conn = ds.getConnection();
			stmt = conn.prepareStatement("UPDATE PROJECTS SET PNAME=?, CONTENT=?, STA_DATE=?,END_DATE=?, STATE=?, TAGS=? WHERE PNO=?");
			stmt.setString(1, project.getTitle());
			stmt.setString(2, project.getContent());
			stmt.setDate(3, new java.sql.Date(project.getStartDate().getTime()));
			stmt.setDate(4, new java.sql.Date(project.getEndDate().getTime()));
	...
	public int delete(int no) throws Exception{
		...
		try {
			conn = ds.getConnection();
			stmt =conn.prepareStatement("DELETE FROM PROJECTS WHERE PNO=?");
			stmt.setInt(1, no);
			return stmt.executeUpdate();
		}catch(Exception e) {
			try {if(stmt!=null) stmt.close();}catch(Exception stmtE) {}
